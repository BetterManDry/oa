...
<div class="panel admin-panel" style="border: 1px solid #ddd;">
	...
	<div class="padding border-bottom">
		<ul class="search">
			<li>
				<a href="${pageContext.request.contextPath}/anno/toAddAnno.a?pageNum=${pageUtil.pageNum}" style="margin-top:10px ;" class="btn btn-success" target="right">添加公告</a>
	...
	<table class="table table-hover text-center">
		...
		<c:forEach items="${annoList}" var="anno">
			<tr>
				<td>
					<input type="checkbox" name="id[]" value="${anno.annoId}" />
				</td>
				<td>${anno.annoId}</td>
				...
				<td>${anno.annoDays}</td>
				<td>
					<a href="${pageContext.request.contextPath}/anno/upAnno.a?pageNum=${pageUtil.pageNum}&annoId=${anno.annoId}" class="btn btn-info" target="right">修改</a>
					<button class="btn btn-warning" onclick="dele(${anno.annoId})" >删除</button>
				</td>
			</tr>
		</c:forEach>
		<tr>
			<td colspan="8">
				<nav aria-label="Page navigation">
					<ul class="pagination">
						<li>
							<a href="${pageContext.request.contextPath}/demo/toAnnoById.a?pageNum=1" aria-label="Previous">
								<span aria-hidden="true">首页</span>
							</a>
						</li>
						<c:if test="${pageUtil.pageNum-1>0}">
							<li>
								<a href="${pageContext.request.contextPath}/demo/toAnnoById.a?pageNum=${pageUtil.pageNum-1}" aria-label="Previous">
									<span aria-hidden="true">上一页</span>
								</a>
							</li>
						</c:if>
						<c:forEach begin="${pageUtil.pageStart}" end="${pageUtil.pageEnd}" var="i" >
							<li><a href="${pageContext.request.contextPath}/demo/toAnnoById.a?pageNum=${i}">${i}</a></li>
						</c:forEach>
						<c:if test="${pageUtil.pageNum+1<pageUtil.pages}">
							<li>
								<a href="${pageContext.request.contextPath}/demo/toAnnoById.a?pageNum=${pageUtil.pageNum+1}" aria-label="Next">
									<span aria-hidden="true">下一页</span>
								</a>
							</li>
						</c:if>
						<li>
							<a href="${pageContext.request.contextPath}/demo/toAnnoById.a?pageNum=${pageUtil.pages}" aria-label="Previous">
